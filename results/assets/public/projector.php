...
if($updated) {
?>
<p style="text-align: left;">Results last synched: <?=date("Y-m-d, h:i:sa (T)", filemtime("assets/cache/".$cname)) ?></p>
<?php
}
...
if(!$updated) {
	...
	foreach($events as $event){
		...
		echo "<td><a href='index.php#",$event['n'],"' >",$event['title'],"</a></td>";
	...
	// output data
	$fs = fopen("assets/cache/".$cname."/".$enum.".event", "r"); $data = "";
	if($fs){
